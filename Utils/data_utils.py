...
def draw(ori_traj, sim_traj, label='sed'):
    ...
    pdf = PdfPages('vis_rlts_geo_sed_batch.pdf')
...
def get_simp_lens(traj_emb,simp_num):


    simp_num = simp_num-traj_emb.shape[0]*2
    var = F.normalize(torch.var(traj_emb.detach(), dim=1, unbiased=False).sum(dim=-1), dim=0)
    #长度控制
    min_len = max(int((simp_num/traj_emb.shape[0])*0),1)
    max_len = min(int((simp_num/traj_emb.shape[0])*2000),1500)
    out = torch.ones((traj_emb.shape[0])).cuda() * (2 + min_len)
    simp_num = simp_num - traj_emb.shape[0] * min_len
    # if simp_num<1500:
    # cnt = torch.multinomial(var, simp_num, replacement=True)
    # idx, counts = torch.unique(cnt, return_counts=True)
    ...
    while simp_num>0:
        ...
        for i in range(idx.size(0)):
            tmp = (out[idx[i]]+counts[i]).item()
            if tmp>=max_len:
                simp_num = simp_num - int((max_len-out[idx[i]].item()))
                out[idx[i]] = max_len
                var[i] = 0
            else:
                simp_num = simp_num - int(counts[i].item())
                out[idx[i]] = tmp


        # idx = torch.multinomial(var, 1).item()
        # if out[idx]<max_len:
        #     out[idx] +=1
        #     simp_num -=1
        # else:
